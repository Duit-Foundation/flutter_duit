...
import 'package:flutter_duit/src/utils/obj.dart';

class NullControllerError extends ArgumentError {
  NullControllerError(String widgetType)
      : super("$widgetType controller is null");
}
  ...
  Widget getWidgetFromElement(DuitElement model) {
    switch (model.type) {
      ...
      case ElementType.singleChildScrollview:
        final it = model
            as SingleChildScrollviewUiElement;
        final child = getWidgetFromElement(it.child);
      ...
      case ElementType.gridView:
        ...
        if (!it.controlled) {
          widgetType =
              GridConstructor.fromValue(it.attributes!.payload["constructor"]);
        } else {
          widgetType = GridConstructor.fromValue(
              it.viewController!.attributes.payload["constructor"]);
        }
      ...
      case ElementType.appBar:
        ...
        return DuitAppBar(
          controller: it.viewController.isNull
              ? throw NullControllerError(ElementType.appBar)
              : it.viewController!,
        );
